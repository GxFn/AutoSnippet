...
function registerRecipesRoutes(app, ctx) {
  ...
  // 允许子目录路径（用 / 分隔），但阻止目录遍历攻击（..）和绝对路径
  if (trimmed.includes('..') || trimmed.startsWith('/') || /^[a-zA-Z]:/.test(trimmed)) {
    return { ok: false, error: 'Invalid recipe name' };
  }
  // 统一使用正斜杠（适配 Windows）
  const normalized = trimmed.replace(/\\/g, '/');
  const rawFileName = normalized.endsWith('.md') ? normalized : `${normalized}.md`;
  if (rawFileName === '.md' || rawFileName === '/.md') return { ok: false, error: 'Invalid recipe name' };
  ...
  try {
    ...
    try {
      ...
      if (!contextModule || !contextModule.getInstance) return;
      const service = contextModule.getInstance(projectRoot);
