...
│  - 用途：Xcode 插入、as:search、AI 检索、as:audit               │
└─────────────────────────────────────────────────────────────────┘
                                    │
...
| **英文名** | **Audit** |
| **中文名** | **代码审计 / 规则检查** |
| **核心功能** | 基于知识库 Recipe 的自动化审计与规则检查 |
| **触发方式** | 通过 `// as:audit` 注释标记（支持 `// as:a` 简写） |
| **技术实现** | 基于 `lib/guard/guardRules.js` 的规则引擎，支持语义检查和静态分析 |
| **作用时机** | 保存后由 watch 触发审计，输出到终端与 Guard 页 |
