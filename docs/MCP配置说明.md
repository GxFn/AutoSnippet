...
| `autosnippet_context_analyze` | 对指定 Recipe 做多维分析（质量、相似度、关系建议），输出结构化结果。 |
| `autosnippet_get_targets` | 获取项目所有 SPM Target 列表。 |
| `autosnippet_get_target_files` | 获取指定 Target 的源码文件列表，需传入 targetName。 |
| `autosnippet_get_target_metadata` | 获取 Target 元数据（package、路径、依赖、sources 等）。 |
| `autosnippet_validate_candidate` | 候选预校验（字段完整性、格式与规范性），返回 errors/warnings/建议。 |
| `autosnippet_check_duplicate` | 候选与现有 Recipe 相似度检测，输出相似条目列表与建议。 |
