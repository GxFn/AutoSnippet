...
| 概念 | 说明 |
|------|------|
| **Recipe** | 知识库的基本单元——一段代码模式 + 使用说明 + 元数据，Markdown 格式存储在 `AutoSnippet/recipes/` |
...
- **冷启动**：Bootstrap 向导——一键扫描 SPM Target，批量生成 Candidate
...
your-project/
├── AutoSnippet/
│   ├── recipes/          # Recipe Markdown 文件（建议 Git 子仓库）
│   └── .autosnippet/     # 索引、候选、Guard 配置等
├── .cursor/
...
| 路径 | 建议 |
|------|------|
| `AutoSnippet/recipes/` | **Git 子仓库**——独立权限控制，仅知识管理员可 push |
| `AutoSnippet/.autosnippet/` | 跟随主仓库——团队共享配置 |
