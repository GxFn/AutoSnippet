...
- **必须用 `###` 二级标题分段**，每个分段单独成行
- **必须用 `-` 或 `*` 建立列表**，每行一个要点
- **必须用 `\n` (换行符) 分隔section和item**，至少 2 行空行分开大分段
- **禁止将所有内容放在一行**（常见AI生成错误）

**❌ BAD Example (禁止这样写):**
```
何时用：在需要…时；与…配合时；或直接拿…做…时。关键点：…内部会…；…支持…，…无需…；找不到…。依赖：…、…（…）。已注册…：…等。
```
**这样会导致用户看到一堵文字墙，无法理解结构。**
...
- `@network_status`：读取当前网络状态
- `@error_handling`：处理网络错误
```
**这样用户能清晰看到每个分段，AI 检索也能更好理解结构。**
...
| **相关 Recipe** | 关联 trigger 或补充模式（如 `@error_handling`） | ⭐⭐ 推荐 |

### 格式与风格建议

- **标题**：使用 `###` 三级标题（Markdown），每个分段独立清晰
- **列表**：用 `-` 或数字列表；避免过长段落
- **代码**：关键代码用 `` ` `` 围绕或 ` ``` ` 代码块
- **强调**：关键词用 `**粗体**`；代码用刻度符
- **对比**：使用 `❌ 错误做法` / `✅ 正确做法` 标记
- **链接**：相关 Recipe 用 `` `@trigger` `` 或 `[@trigger](link)` 格式
...
- **易错点 1**：具体说明及规避
- **线程安全**：是否线程安全、何时需要锁定
- **版本限制**：最低 iOS X.Y+，高版本 API 优先

### 常见误用

- ❌ 在主线程执行长时间操作 → ✅ 后台线程处理，主线程回调
- ❌ 忽略中断信号 → ✅ 及时检查并取消

### 相关 Recipe

- `@asynctask`：异步任务处理的标准写法
- `@thread_safety`：线程安全的最佳实践
```
