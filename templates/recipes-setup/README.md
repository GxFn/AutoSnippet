...
| 工具 | 说明 |
|------|------|
| `asd_search` | 统合搜索（mode=auto: FieldWeighted + 语义融合，mode=context: 智能上下文检索） |
| `asd_submit_knowledge` | 统一知识提交（单条/批量/文档）。使用 `items` 数组格式传入 1~N 条。文档模式设 `knowledgeType: 'dev-document'` 仅需 title + markdown |
| `asd_knowledge(operation=confirm_usage)` | 确认 Recipe 被采纳/应用 |
| `asd_knowledge(operation=insights)` | 获取 Recipe 质量洞察（只读） |
