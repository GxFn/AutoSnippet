...
/* ═══ 3. Bootstrap Phase 1.8 panoramaResult in return ══════ */

describe('bootstrap-phases Phase 1.8 panoramaResult', () => {
  it('PhaseReport type includes panorama phase in doc header', async () => {
    // Read the file comments to verify Phase 1.8 is documented
    ...
    expect(content).toContain('Phase 1.8');
    expect(content).toContain('Panorama');
    expect(content).toContain('panoramaResult');
  });
});

/* ═══ 4. Panorama context builder ═══════════════════════════ */

describe('buildPanoramaContext in orchestrator', () => {
  it('orchestrator contains panorama context builder', async () => {
    const fs = await import('node:fs');
    const content = fs.readFileSync(
      new URL(
    ...
    expect(content).toContain('function buildPanoramaContext');
    // Verify it's used in strategyContext
    expect(content).toContain('panorama: buildPanoramaContext(fillContext, dimConfig)');
  });
});
