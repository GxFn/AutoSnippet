...
/**
 * 语义搜索（需 embed 索引）
 * @param {Object} options.filter - { language?, category? } 过滤条件
 */
async function semanticSearch(projectRoot, keyword, limit = 5, filter = {}) {
	try {
		...
		const mergedFilter = { type: 'recipe', ...filter };
		const items = await service.search(keyword, { limit, filter: mergedFilter });
...
/**
 * 统一入口：关键词 + 可选语义
 * @param {Object} options.filter - { language?, category? } 过滤条件，仅语义搜索时生效
 */
async function search(projectRoot, keyword, options = {}) {
	const { semantic = false, limit = 10, filter = {} } = options;
	if (semantic) {
		const semanticResults = await semanticSearch(projectRoot, keyword, limit, filter);
