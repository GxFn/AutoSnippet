...
/**
 * DeepSeek 辅助搜索（基于 keyword + ranking 候选）
 * 从两类候选中挑选 Top N 结果加入返回池
 * 
 * 失败策略：AI 请求失败直接返回 []，不重试（混合搜索中 ranking/keyword 已有结果）
 */
async function performDeepseekAssistSearch(keyword, rankingCandidates, keywordCandidates, limit = 3) {
  try {
...
请从中选出最相关的前 ${limit} 个结果。只返回编号，用逗号分隔（例如：1,5,8）：`;
    ...
    // 为了避免 AI 失败时的长时间等待，直接调用 chat 而不走 _withRetry
    // 失败时直接放弃 AI 辅助，混合搜索已有 ranking/keyword 结果
    let response;
    try {
      response = await ai.chat(prompt, [], '');
    } catch (chatError) {
      // AI 失败不重试，直接返回空数组
      console.debug('[DeepSeek Assist] AI 调用失败，放弃辅助结果:', chatError.message);
      return [];
    }
    ...
    // 其他错误（如候选合并）放弃 AI
    console.debug('[DeepSeek Assist] 失败:', error.message);
    return [];
  }
}
...
async function performHybridSearch(projectRoot, keyword, options = {}) {
  ...
  // DeepSeek 从 key + rank 候选中挑选 3 条辅助结果
  // 可通过 enableAiAssist 选项或环境变量禁用（如 AI 服务故障时）
  const aiResults = aiEnabled ? (await performDeepseekAssistSearch(keyword, rankingAll, keywordAll, topN)) : [];
...
module.exports = { performUnifiedSearch, performHybridSearch, performDeepseekAssistSearch };
