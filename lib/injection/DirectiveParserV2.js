...
class DirectiveParserV2 {
  constructor(config = {}) {
    ...
    this.HEADER_PATTERN_QUOTED = /^"([A-Za-z0-9_+./-]+\.h)"(?:\s+(.+))?$/;
  }
  ...
  createHeader(headerLine) {
    try {
      ...
      // 匹配 "Header.h" 以及可选的相对路径
      const quoted = content.match(this.HEADER_PATTERN_QUOTED);
      if (quoted) {
        const headerName = quoted[1];
        const headRelativePathFromMark = quoted[2] || null;
        return {
          name: `"${headerName}"`,
          specName: `"${headerName}"`,
          moduleName: '',
          headerName,
          moduleStrName: `"${headerName}"`,
          headerStrName: `"${headerName}"`,
          headRelativePathFromMark
        };
      }
