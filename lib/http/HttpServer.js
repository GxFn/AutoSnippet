...
export class HttpServer {
  ...
  setupMiddleware() {
    ...
    this.app.use((req, res, next) => {
      const isLongRunning = req.path.includes('/spm/scan') || req.path.includes('/extract/');
      req.setTimeout(isLongRunning ? 300000 : 60000); // AI 扫描 5分钟，其他 60秒
      next();
    });
  }
  ...
  setupRoutes() {
    ...
    this.app.get('/', (req, res) => {
      res.json({ service: 'AutoSnippet Dashboard', status: 'running' });
    });

    // ── 兼容无版本前缀 /api/health ──
    this.app.use('/api/health', healthRouter);
    this.app.use('/api/candidates', candidateRouter);
