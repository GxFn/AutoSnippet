 ...
 * 通过 ChatAgent bootstrap_full_pipeline 任务流执行:
 *   1. 加载 coldstart Skill（操作指南）
 *   2. 执行 bootstrap_knowledge（SPM 扫描 + 候选创建）
 *   3. 根据主语言加载 language-reference Skill
 *   4. 可选 AI 语义补齐（autoEnrich，默认 true）
 *   5. 可选 AI 内容润色（autoRefine，默认 false）
 */
router.post('/bootstrap', asyncHandler(async (req, res) => {
  const { maxFiles, skipGuard, contentMaxLines, autoEnrich, autoRefine, refinePrompt } = req.body || {};
