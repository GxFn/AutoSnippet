 ...
 * 冷启动：结构收集 + 9 维度 Candidate 创建（与 MCP bootstrap_knowledge 一致）
 * 不使用项目内 AI，纯结构扫描 + 启发式提取
 */
router.post('/bootstrap', asyncHandler(async (req, res) => {
  ...
  logger.info('Bootstrap cold start initiated via dashboard');
  ...
  const result = await bootstrapKnowledge(ctx, args);

  // bootstrapKnowledge 返回 envelope 格式，提取 data
