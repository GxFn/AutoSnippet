...
export class KnowledgeGraphService {
   ...
   */
  getStats() {
    const edgeCount = this.db.prepare(`SELECT COUNT(*) as total FROM knowledge_edges`).get();
    const byRelation = this.db.prepare(
      `SELECT relation, COUNT(*) as count FROM knowledge_edges GROUP BY relation`
    ).all();
    const byType = this.db.prepare(
      `SELECT from_type as type, COUNT(DISTINCT from_id) as count FROM knowledge_edges GROUP BY from_type
       UNION
       SELECT to_type as type, COUNT(DISTINCT to_id) as count FROM knowledge_edges GROUP BY to_type`
    ).all();
   ...
   */
  getAllEdges(limit = 500) {
    const rows = this.db.prepare(
      `SELECT * FROM knowledge_edges ORDER BY updated_at DESC LIMIT ?`
    ).all(limit);
    return rows.map(this._mapEdge);
  }
