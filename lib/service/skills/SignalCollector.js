...
export class SignalCollector {
  ...
  async #tick() {
    ...
    try {
      ...
      // 3. 调用 ChatAgent AI 分析
      this.#logger.debug('[SignalCollector] invoking ChatAgent for analysis...');
      const { reply, toolCalls } = await this.#chatAgent.execute(prompt, { history: [] });
  ...
  #collectGuardSignals() {
    try {
      ...
      const rows = this.#db.prepare(
        `SELECT name, type, conflict_count, last_conflict_at
         FROM skills WHERE conflict_count > 0
         ORDER BY last_conflict_at DESC LIMIT 20`
      ).all();
      return rows;
    } catch { return []; }
  }
  ...
  #collectRecipeSignals() {
    try {
      if (!this.#db) return [];
      const rows = this.#db.prepare(
        `SELECT id, name, type, use_count, success_count, fail_count, updated_at
  ...
  #collectRecentActions() {
    try {
      if (!this.#db) return [];
      const since = this.#snapshot.lastRun || new Date(Date.now() - 24 * 3600 * 1000).toISOString();
      const rows = this.#db.prepare(
        `SELECT actor, action, target_type, result, timestamp
