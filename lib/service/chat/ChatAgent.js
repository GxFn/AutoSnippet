...
export class ChatAgent {
  ...
  /** @type {string} v10 提示词模式: 'full-signal' (v9) | 'minimal-prompt' (v10 领域大脑) */
  #promptMode = 'full-signal';
   ...
   */
  async execute(prompt, { history = [], conversationId, source = 'user', budget: budgetOverrides, dimensionId, dimensionMeta, promptMode } = {}) {
    this.#currentSource = source;
    this.#promptMode = promptMode || 'full-signal';
  ...
  async #executeWithNativeTools(prompt, { effectiveHistory, conversationId, source, execStartTime, budget = DEFAULT_BUDGET, dimensionMeta }) {
    ...
    // Bootstrap 场景限制可用工具集
    // v10: 领域大脑模式增加 list_project_structure / get_file_summary / semantic_search_code
    const promptMode = opts?.promptMode || 'full-signal'; // 'full-signal' | 'minimal-prompt'
...
5. 当工具返回错误时，尝试不同参数或方法。`;
    ...
    // v10: 领域大脑模式 — LLM 是领域专家，自主决策探索路径
    if (this.#promptMode === 'minimal-prompt') {
      return `你是「领域大脑」— 你对软件工程领域拥有深厚的专家知识。
你将分析一个真实项目，自主发现其中有价值的代码知识。
${this.#projectBriefingCache}

## 你的身份
你不是一个执行指令的工具，你是具备深度技术洞察力的领域专家。
你知道什么样的代码知识对开发团队最有价值 — 不是显而易见的样板代码，
...
## 「项目特写」风格——最高优先级
submit_candidate 的 code 字段必须用「项目特写」风格:
- **描述与代码交织**: 不是纯代码也不是纯说明，而是两者融合
- **使用项目真实类名/前缀**: 读完即知「在本项目中」该怎么写
- **标题用“— 项目特写”后缀**: 标识文档风格
- **标注代码来源**: (来源: FileName.m:42)
...
- 高效利用步数 (≤${budget.maxIterations} 轮)`;
    ...
    // Bootstrap 系统模式 (v9 full-signal): 精简提示词（~400 token）
