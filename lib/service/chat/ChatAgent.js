...
请用 JSON 数组格式返回建议: [{"violation": "...", "suggestion": "...", "fixExample": "..."}]`;
   ...
   *
   * 流程:
   *   1. 加载 coldstart Skill → 获取 9 维度分析指南
   *   2. 执行 bootstrap_knowledge → SPM 扫描 + 候选创建
   *   3. 根据主语言加载对应 language-reference Skill
   *   4. 如 AI 可用且 autoRefine=true → 用 Skill 上下文润色候选
   *
   * @param {object} opts
   * @param {number} opts.maxFiles 最大扫描文件数
   * @param {boolean} opts.skipGuard 是否跳过 Guard
   * @param {number} opts.contentMaxLines 每文件最大行数
   * @param {boolean} opts.autoRefine 是否自动执行 AI 润色 (Phase 6)
   */
  async #taskBootstrapPipeline({ maxFiles, skipGuard, contentMaxLines, autoRefine = false } = {}) {
    ...
    // ── Step 1: 加载 coldstart Skill ──────────────────────
    this.#logger.info('[BootstrapPipeline] Step 1: Loading coldstart skill');
    try {
      const skill = await this.executeTool('load_skill', { skillName: 'autosnippet-coldstart' });
      if (skill && !skill.error) {
        pipelineResult.skillsLoaded.push('autosnippet-coldstart');
      }
    } catch { /* skill loading is best-effort */ }

    // ── Step 2: 执行 bootstrap_knowledge ──────────────────
    this.#logger.info('[BootstrapPipeline] Step 2: Running bootstrap_knowledge');
    ...
    // ── Step 3: 根据主语言加载 reference Skill ─────────────
    const primaryLang = bootstrapResult?.primaryLanguage || '';
    const langSkillMap = {
    ...
    const langSkillName = langSkillMap[primaryLang.toLowerCase()];
    let langSkillContent = null;

    if (langSkillName) {
      this.#logger.info(`[BootstrapPipeline] Step 3: Loading language skill ${langSkillName}`);
      try {
        const langSkill = await this.executeTool('load_skill', { skillName: langSkillName });
        if (langSkill && !langSkill.error) {
          pipelineResult.skillsLoaded.push(langSkillName);
          langSkillContent = langSkill.content;
        }
      } catch { /* best-effort */ }
    }

    // ── Step 4: 可选 — AI 润色 (Phase 6) ────────────────────
    const candidatesCreated = bootstrapResult?.bootstrapCandidates?.created || 0;
    if (autoRefine && candidatesCreated > 0 && this.#aiProvider) {
      this.#logger.info(`[BootstrapPipeline] Step 4: Auto-refining ${candidatesCreated} candidates`);
      try {
        const refinePrompt = langSkillContent
          ? `请参考以下业界最佳实践标准润色候选，确保 summary 精准、tags 丰富、confidence 合理:\n${langSkillContent.substring(0, 3000)}`
          : undefined;
