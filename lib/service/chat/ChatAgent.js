...
请用 JSON 数组格式返回建议: [{"violation": "...", "suggestion": "...", "fixExample": "..."}]`;
   ...
   *   Phase 0: bootstrap（SPM 扫描 + Skill 增强维度 + 候选创建）
   *   Phase 1: enrich（结构补齐，依赖 bootstrap 产出的候选 ID）
   *   Phase 1: loadSkill（并行加载语言参考 Skill，用于润色提示）
   *   Phase 2: refine（AI 润色，依赖 enrich + loadSkill）
   */
    ...
    this.registerPipeline(new TaskPipeline('bootstrap_full_pipeline', [
      ...
      },
      {
        ...
        when: (ctx) => {
          ...
          return !!autoRefine && created > 0 && hasAI;
        },
        errorStrategy: 'continue',
      },
    ]));
