...
请用 JSON 数组格式返回建议: [{"violation": "...", "suggestion": "...", "fixExample": "..."}]`;
   ...
   * bootstrapKnowledge() 只做启发式 Phase 1-5.5，不调 AI。
   * Phase 5.5 为宏观维度自动生成 Project Skills（architecture/code-standard/project-profile/agent-guidelines）。
   * AI 增强步骤由 ChatAgent DAG 编排:
   *
   * bootstrap_full_pipeline:
   *   Phase 0: bootstrap（SPM 扫描 + Skill 增强维度 + 候选创建 + 宏观 Skill 自动生成，纯启发式）
