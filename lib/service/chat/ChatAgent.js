  ...
  getCapabilities() {
    return {
      ...
      tasks: [
        ...
        { name: 'bootstrap_full_pipeline', description: '冷启动全流程 (DAG): bootstrap → enrich → refine' },
      ],
...
请用 JSON 数组格式返回建议: [{"violation": "...", "suggestion": "...", "fixExample": "..."}]`;
   ...
   *   单步调用 bootstrap_knowledge，所有 Phase（1-7）均在共享 handler 内完成。
   *   ChatAgent 和 MCP 外部 Agent 走完全相同的链路，质量一致。
   *
   *   Phase 1-5: 启发式扫描 + 候选创建（无需 AI）
   *   Phase 6: autoEnrich — AI 语义补齐（需要 aiProvider）
   *   Phase 7: autoRefine — AI 内容润色（需要 aiProvider）
   */
    ...
    this.registerPipeline(new TaskPipeline('bootstrap_full_pipeline', [
      {
        ...
        params: {
          ...
          loadSkills: true,
          autoEnrich: true,
          autoRefine: (ctx) => ctx._inputs.autoRefine ?? true,
          refinePrompt: (ctx) => ctx._inputs.refinePrompt,
        },
      },
    ]));
  }
