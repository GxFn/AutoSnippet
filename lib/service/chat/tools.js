 ...
 * │ 10. extract_recipes      从源码提取 Recipe 结构   │
 * │ 11. enrich_candidate     AI 语义字段补全         │
...
const enrichCandidate = {
  ...
  description: 'AI 语义字段补全 — 分析候选代码，自动填充缺失的 rationale/knowledgeType/complexity/scope/steps/constraints。',
  parameters: {
    type: 'object',
    properties: {
...
const refineBootstrapCandidates = {
  ...
  description: 'Phase 6 AI 润色 — 对 Bootstrap 冷启动生成的候选进行二次精炼：改善描述、补充架构洞察、推断 relations 关联、调整 confidence 评分。',
  parameters: {
    type: 'object',
    properties: {
  ...
  handler: async (params, ctx) => {
    ...
    return candidateService.refineBootstrapCandidates(
      ctx.aiProvider,
      { candidateIds: params.candidateIds, dryRun: params.dryRun },
      { userId: 'agent' },
    );
  },
};
