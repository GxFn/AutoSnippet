...
const bootstrapKnowledgeTool = {
  ...
  description: '一键冷启动知识库初始化: SPM Target 扫描 → 依赖图谱 → Guard 审计 → 9 维度 Candidate 自动创建。支持 Skill 增强维度定义。适用于空项目首次建库。',
  parameters: {
    type: 'object',
    properties: {
      ...
      autoEnrich: { type: 'boolean', description: '自动 AI 补齐候选语义字段（Phase 6），默认 true' },
      autoRefine: { type: 'boolean', description: '自动 AI 润色候选内容（Phase 7），默认 true' },
      refinePrompt: { type: 'string', description: '自定义润色提示词（仅 autoRefine=true 时生效）' },
    },
  },
  handler: async (params, ctx) => {
    ...
    const result = await bootstrapKnowledge(
      { container: ctx.container, logger },
      {
        ...
        autoEnrich: params.autoEnrich ?? true,
        autoRefine: params.autoRefine ?? true,
