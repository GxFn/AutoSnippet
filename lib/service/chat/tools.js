 ...
 * │ 33. suggest_skills        推荐创建 Skill          │
 * │ 34. bootstrap_knowledge   冷启动知识库初始化      │
 * └─────────────────────────────────────────────────────┘
 * ┌─── 组合工具 (3) ───────────────────────────────────┐
 * │ 35. analyze_code          Guard + Recipe 搜索      │
 * │ 36. knowledge_overview    全局知识库概览           │
 * │ 37. submit_with_check     查重 + 提交              │
 * └─────────────────────────────────────────────────────┘
 */
...
// ────────────────────────────────────────────────────────────
// 33. suggest_skills — 基于使用模式推荐 Skill 创建
// ────────────────────────────────────────────────────────────
const suggestSkills = {
  name: 'suggest_skills',
  description: '基于项目使用模式分析，推荐创建 Skill。分析 Guard 违规频率、Memory 偏好积累、Recipe 分布缺口、候选积压率。返回推荐列表（含 name/description/rationale/priority），可据此直接调用 create_skill 或 autosnippet_create_skill 创建。',
  parameters: {
    type: 'object',
    properties: {},
    required: [],
