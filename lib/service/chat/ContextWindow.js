...
export class PhaseRouter {
  ...
  update(roundResult) {
    ...
    if (this.#phase === 'PRODUCE') {
      ...
      // 搜索预算+容忍期都耗尽且 0 提交 → 强制进入 SUMMARIZE
      const totalSearchRounds = this.#totalIterations; // 简化: 用总轮次近似
      if (totalSearchRounds >= this.#budget.searchBudget + this.#budget.searchBudgetGrace && this.#totalSubmits === 0) {
        this.#logger.info(`[PhaseRouter] search budget + grace exhausted, 0 submits → SUMMARIZE`);
        return this.#transitionTo('SUMMARIZE');
      }
