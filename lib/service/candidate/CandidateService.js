...
export class CandidateService {
   ...
   *
   * 对 source='bootstrap' 的候选逐条调用 AI，改善描述、补充洞察、推断 relations、
   * 调整 confidence。不会删除/合并候选——仅原地更新。
   *
  ...
  async refineBootstrapCandidates(aiProvider, options = {}, context = { userId: 'system' }) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        const prompt = this._buildRefinePrompt(candidate, allTitles);
   ...
   * @private
   */
  _buildRefinePrompt(candidate, allTitles) {
    const meta = candidate.metadata || {};
