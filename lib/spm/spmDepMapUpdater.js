...
function updateSpmDepMap(projectRoot, options = {}) {
	const mapPath = path.join(projectRoot, MAP_FILE);
	...
	for (const p of packageSwiftFiles) {
		let src = '';
		try { src = fs.readFileSync(p, 'utf8'); } catch { continue; }
		discovered.products.push(...extractProductUsages(src));
	...
	// 3) graph/policy：全项目依赖图 + 约束（不新增文件，写回 spmmap）
	const graph = buildSpmProjectGraph(projectRoot, packageSwiftFiles);
	...
	merged.changed = true; // graph 每次都会刷新 generatedAt；按 always 策略落盘
	...
	// 如果文件不存在，先写入空模板（与 root/watch ensure 逻辑一致）
