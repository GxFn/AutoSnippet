...
function readSpmDepMap(specFile) {
	...
	try {
		...
		const mapPath = path.join(rootDir, SPM_DEP_MAP_FILE);
		let raw = null;
		try {
			raw = fs.readFileSync(mapPath, 'utf8');
		} catch {
			// 兼容一次迁移：若新文件不存在，尝试读取旧文件
			const oldPath = path.join(rootDir, SPM_DEP_MAP_FILE_OLD);
			raw = fs.readFileSync(oldPath, 'utf8');
		}
		if (!raw) return null;
		return JSON.parse(raw);
	} catch {
