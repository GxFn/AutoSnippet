...
async function handleSearchTrigger(specFile, fullPath, relativePath, searchLine) {
	...
	// 优先语义搜索，失败则关键词搜索
	const results = await searchService.search(projectRoot, keyword, { semantic: true, limit: 8 });
	if (results.length === 0) {
		console.log('[as:search] 未找到匹配内容');
		return;
	}
...
async function handleGuardTrigger(specFile, fullPath, code, guardLine) {
	...
	const keyword = guardLine.replace(/^\/\/\s*as:(?:guard|g)\s*/, '').trim();
