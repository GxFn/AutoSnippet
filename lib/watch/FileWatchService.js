...
class FileWatchService {
  ...
  // 异步记录窗口上下文（不堵塞防抖逻辑）
  this.windowContextManager.recordWindowContext(fullPath).catch(err => {
    if (process.env.ASD_DEBUG === '1') {
    console.debug(`[Watch] 记录窗口信息失败: ${err.message}`);
    }
  });
  ...
  fs.access(updateFile, fs.constants.F_OK, (err) => {
    ...
    fs.readFile(updateFile, 'utf8', async (readErr, data) => {
      ...
      if (triggers.searchLine) {
      // 将窗口上下文传递给搜索处理器
      const windowContext = this.windowContextManager.savedWindowContexts.get(updateFile);
      await SearchHandler.handle(specFile, updateFile, relativePath, triggers.searchLine, { windowContext });
      }
