...
class SearchHandler {
  ...
  async handle(specFile, fullPath, relativePath, searchLine) {
    ...
    return automationOrchestrator.run(
      {
        type: 'search',
        handler: (context) => this._handleSearch(context)
      },
  ...
  _tryAutoPasteXcode(lineCount = 0) {
    try {
      ...
      const args = [
        ...
        // 现在光标在该行行首，需要移到上一行的末尾，然后创建新行并粘贴
        '-e', 'key code 126',  // Up 移到上一行
        '-e', 'delay 0.1',
        '-e', 'key code 124 using command down',  // Cmd+Right 移到行末
        '-e', 'delay 0.1',
