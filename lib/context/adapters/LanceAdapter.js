...
class LanceAdapter {
	...
	_applyFilter(rows, filter) {
		...
		if (!filter.language && !filter.category) return rows;
	...
	async searchVector(queryVector, options = {}) {
		...
		if (hasVector) {
			...
			let q = searchMethod.call(table, queryVector).limit(limit * 3);
			if (whereClause && q.where) q = q.where(whereClause);
			let rows = await (q.toArray ? q.toArray() : q.execute());
			rows = this._applyFilter(rows, filter);
