...
function applySpecDefaults(specObj, specFile) {
	...
	if (base === defaults.ROOT_SPEC_FILENAME) {
		...
		if (!specObj.recipes || typeof specObj.recipes !== 'object') {
			specObj.recipes = { dir: defaults.RECIPES_DIR, format: 'md+frontmatter', index: defaults.RECIPES_INDEX };
		} else {
...
function writeSpecFile(specFile, obj) {
	...
	if (baseName === defaults.ROOT_SPEC_FILENAME && Array.isArray(obj.list)) {
		// 分离所有片段到独立文件
