...
function applySpecDefaults(specObj, specFile) {
	...
	if (base === 'AutoSnippetRoot.boxspec.json') {
		...
		if (!specObj.skills || typeof specObj.skills !== 'object') {
			specObj.skills = { dir: 'Knowledge/skills', format: 'md+frontmatter', index: 'Knowledge/skills/index.json' };
		} else {
			if (!specObj.skills.dir) specObj.skills.dir = 'Knowledge/skills';
			if (!specObj.skills.format) specObj.skills.format = 'md+frontmatter';
			if (!specObj.skills.index) specObj.skills.index = 'Knowledge/skills/index.json';
		}
