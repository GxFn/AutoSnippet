...
function writeSingleSnippet(snippet, template) {
	...
	if (snippet.headName) {
		...
		// 模板用 {content} 取的是 body，必须把带 as:include 的行写入 body 才能写到 .codesnippet
		const baseLines = Array.isArray(extPlace.body) ? extPlace.body : (Array.isArray(extPlace.content) ? extPlace.content : []);
		let array = [];
		if (extPlace.languageShort === 'swift') {
			array = ['// autosnippet:import ' + header, '', ...baseLines];
		} else {
			array = ['// autosnippet:include ' + header, '', ...baseLines];
		}
		extPlace.body = array;
