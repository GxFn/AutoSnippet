...
const DEFAULT_RULES = {
	...
	'ui-off-main-swift': {
		message: 'UIKit/UI 相关调用应在主线程执行',
		severity: 'warning',
		pattern: 'UIView\\.init|UIApplication\\.shared|UILabel\\(|UIButton\\(|DispatchQueue\\.main\\.async',
		languages: ['swift'],
		note: '仅作简单模式提示',
		dimension: 'file'
	},
...
function runCodeChecks(projectRoot, code, language, scope) {
	...
	for (const lineIdx of returnNilLineIndices) {
		...
		if (/\[super\s+init\]|self\s*=\s*\[super\s+init\]/.test(slice)) continue;
	...
	// Block 内直接 self. 且块内无 __weak/weakSelf：按 ^{ ... } 范围判断
...
function runStaticCheck(projectRoot, code, language, scope) {
	...
	// KVO 文件级检查：有 addObserver 无 removeObserver 时提示（静态难做全，仅简单扫描）
	if (language === 'objc' && (code || '').includes('addObserver')) {
		const hasRemove = /\bremoveObserver\b/.test(code || '');
		if (!hasRemove) {
