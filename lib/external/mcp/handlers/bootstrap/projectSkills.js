 ...
 * @returns {string} Skill Markdown 内容（不含 frontmatter）
 */
...
function _buildCategoryScanSkill(candidates, context) {
  ...
  for (const c of candidates) {
    ...
    lines.push(`## ${heading || c.subTopic || 'Category'}`, '');
    if (c.summary) lines.push(`> ${c.summary}`, '');
    if (methodSigs.length > 0) {
      for (const sig of methodSigs) lines.push(`- ${sig}`);
      lines.push('');
    }
    lines.push(
