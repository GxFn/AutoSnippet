...
export function _extractProjectProfile(allFiles, targetFileMap, depGraphData, guardAudit, langStats, lang, ast, pipelineCtx) {
  ...
  // ── tech-stack-and-conventions（v4.1 新增：深度项目特征提取）──
  {
    ...
    for (const name of classNames) {
      ...
      if (m) {
        let domain = m[1];
        // Fix: 只在前缀后跟 大写+小写 边界时才剥离（避免 BDVideo → ideoPlayer 的误剥离）
        domain = domain.replace(/^[A-Z]{2,3}(?=[A-Z][a-z])/, '');
        if (domain.length >= 2) {
  ...
  if (depGraphData?.nodes) {
    ...
    // Fix: 排除系统框架 — 它们不是真正的三方依赖
    const SYSTEM_FRAMEWORKS = new Set([
      'foundation', 'uikit', 'swiftui', 'combine', 'coredata', 'coregraphics',
      'corelocation', 'mapkit', 'avfoundation', 'webkit', 'security', 'photos',
      'corebluetooth', 'coremotion', 'storekit', 'usernotifications', 'cloudkit',
      'gamekit', 'spritekit', 'scenekit', 'arkit', 'metalkit', 'metal',
      'coreml', 'vision', 'naturallanguage', 'cryptokit', 'os', 'darwin',
      'dispatch', 'objectivec', 'coreservices', 'systemconfiguration',
      'corefoundation', 'quartzcore', 'coreimage', 'contacts', 'eventkit',
      'healthkit', 'homekit', 'passkit', 'pushkit', 'safariservices',
      'authenticationservices', 'networkextension', 'network', 'iokit',
      'accelerate', 'medialibrary', 'mediaplayer', 'audiounit', 'audiotoolbox',
      'coretext', 'xctestui', 'xctest', 'swift', 'swiftonconedata',
      // 常见伪依赖（SPM target 名 = 项目内子模块,但不在 projectTargets 中的情况）
      'ui', 'business', 'services',
    ]);
    const thirdPartyNodes = (depGraphData.nodes || [])
      .map(n => typeof n === 'string' ? n : n.id || n.label)
      .filter(n => !projectTargets.has(n.toLowerCase()) && !SYSTEM_FRAMEWORKS.has(n.toLowerCase()));
  ...
  // ── base-extensions（v4.2 新增：Extension/Category 分类聚合）──
  {
    ...
    // Fix: 过滤掉 include/ 等三方/重复头文件目录,避免扩展计数翻倍
    const extScanFiles = allFiles.filter(f => !isThirdParty(f));
