...
const PROJECT_SNAPSHOT_STYLE_GUIDE = `# 「项目特写」风格指南 — 最高优先级

submit_candidate 的 code 字段必须严格遵循「项目特写」风格:

## 什么是「项目特写」
「项目特写」是描述与代码**交织融合**的 Markdown 文档。它不是纯代码也不是纯文档，
而是让读者**读完即知「在本项目中该怎么写」**的一站式参考。

## 风格要求
1. **标题**: 使用 "## XXX — 项目特写" 格式
2. **描述与代码交织**: 先一段解释(该模式是什么、为什么这样做)，再跟对应的代码
3. **使用项目真实类名/前缀**: 不用 \`MyClass\`、\`SomeService\` 等占位名
4. **标注代码来源**: 代码块后标注 \`(来源: FileName.m:行号)\`
5. **包含使用场景**: 说明在什么情况下使用这个模式
6. **数据佐证**: 引用具体的文件数量、使用频次等数字
...
const FEWSHOT_CANDIDATE_EXAMPLE = `# Few-shot 示例 — 优秀的「项目特写」候选
...
XY 项目全局统一使用 \`dispatch_once\` 风格的单例。
项目中共有 **12 个单例类**，均遵循以下模板:
...
### 使用场景
在 ViewController 或 Service 中通过 \`[XYNetworkManager sharedInstance]\` 获取实例，
...
- ✅ 标注了来源文件和行号
- ✅ 说明了使用场景和禁止事项
- ✅ 给出了具体数字 (12 个单例类) 和变体列表`;

const FEWSHOT_DEEP_SCAN_EXAMPLE = `# Few-shot 示例 — 深度扫描类「项目特写」候选
...
### 使用约束
- 所有网络请求必须使用 \`kXYBaseURL\`，不可硬编码域名
- UI 布局必须使用预定义尺寸宏，适配刘海屏
\`\`\`
...
5. 可选: semantic_search_code 在知识库中查找相关知识`);
  ...
  // ── Section 6: 「项目特写」风格定义 + Few-shot 示例 ──
  if (outputType !== 'skill') {
...
- **code**: 「项目特写」风格 Markdown（见上方风格指南和示例）
- **title**: "[Bootstrap] ${dim.id}/子主题"
- **summary**: ≤80字，引用真实类名和数字
- **language**: 按项目实际语言
- **category**: View/Service/Tool/Model/Network/Storage/UI/Utility
...
- 「项目特写」风格是最高优先级 — 每条候选必须让读者一眼知道「在本项目中该怎么写」`);
