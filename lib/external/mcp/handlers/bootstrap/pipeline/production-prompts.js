...
export function buildMinimalPrompt(dim, context, opts = {}) {
  const { isRecalculation = false, existingCandidates = [], budget, signalCount } = opts;
  const parts = [];

  // ── Section 1: 维度任务与探索目标 ──
  const signalHint = signalCount > 0
    ? `\n\n> 💡 启发式预扫描检测到 **${signalCount}** 个相关信号，这个维度在本项目中有较丰富的内容可挖掘。`
    : '';
  parts.push(`# 维度任务: ${dim.label} (${dim.id})
${dim.guide}

## 探索目标
${buildExplorationGoals(dim)}${signalHint}`);
...
6. **关键**: 探索完成后立即调用 submit_candidate 提交候选 — 不要只分析不提交`);
