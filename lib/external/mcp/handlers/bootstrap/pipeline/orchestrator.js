...
import { buildDimensionProductionPrompt } from './production-prompts.js';
...
export async function fillDimensionsAsync(fillContext) {
  ...
  for (const dimId of DIMENSION_EXECUTION_ORDER) {
    ...
    try {
      ...
      // 构建生产 prompt（携带预算配置）
      const prompt = buildDimensionProductionPrompt(dim, signals, contextSnapshot, { budget: dimBudget });
