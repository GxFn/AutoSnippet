...
export async function fillDimensionsAsync(fillContext) {
  ...
  for (const dimId of DIMENSION_EXECUTION_ORDER) {
    ...
    try {
      ...
      // v10: 根据 promptMode 选择 prompt 构建策略
      // 可通过环境变量 ASD_PROMPT_MODE='minimal-prompt' 切换
      const promptMode = process.env.ASD_PROMPT_MODE || 'full-signal';
      let prompt;
      if (promptMode === 'minimal-prompt') {
        prompt = buildMinimalPrompt(dim, contextSnapshot, { budget: dimBudget });
        ctx.logger.info(`[Bootstrap] Using v10 minimal-prompt for "${dimId}"`);
      } else {
