...
export function _extractCodePattern(allFiles, lang, ast, projectPrefix = '') {
  ...
  for (const [patternName, pDef] of Object.entries(patternVariants)) {
    ...
    // 首选写法提示（跳过 boilerplate 变体）
    const primaryVariant = scanResult.variants.find(v => !v.boilerplate) || scanResult.variants[0];
    const primaryLabel = primaryVariant ? primaryVariant.label : pDef.label;
    const agentNotes = [
      `你在本项目中实现${pDef.label}时，**必须**使用 ${primaryLabel} 写法（${primaryVariant?.fileCount || 0} 个文件使用此写法）`,
      `新代码 **必须** 遵循项目首选的 ${primaryLabel} 写法`,
    ];
    if (scanResult.variants.length > 1) {
      agentNotes.push(`项目中有 ${scanResult.variants.length} 种${pDef.label}写法 — 新代码统一使用首选写法`);
    }
    ...
    results.push({
      ...
      tags: [patternName],
      // v7: 保留结构化 scanResult 供 signal-extractor 直接使用，避免从 Markdown 反向提取
...
export function _extractBestPractice(allFiles, lang, projectPrefix = '') {
  ...
  for (const [key, pattern] of Object.entries(patterns)) {
    ...
    const agentNotes = [
      `你在本项目中实现${pattern.label}时，**必须**使用 ${primaryLabel} 写法（${primaryVariant?.fileCount || 0} 个文件使用此写法）`,
      `新代码 **必须** 遵循项目首选的 ${primaryLabel} 写法`,
    ];
    if (scanResult.variants.length > 1) {
...
function _extractLogging(allFiles, lang, results) {
  ...
  results.push({
    ...
    code: _buildCandidateDoc({
      ...
      bodyLines,
      agentNotes: [
        `你在本项目中添加日志时，**必须**使用 ${primaryLabel}`,
        scanResult.variants.length > 1
          ? `项目中有 ${scanResult.variants.length} 种日志方式 — 新代码统一使用 ${primaryLabel}`
          : '项目日志方式统一',
      ],
    }),
    language: lang,
...
function _extractTesting(allFiles, lang, results) {
  ...
  results.push({
    ...
    code: _buildCandidateDoc({
      ...
      bodyLines,
      agentNotes: [
        `你在本项目中编写测试时，**必须**使用 ${primaryLabel} 并遵循以上项目测试写法`,
        '测试方法名应清晰描述测试意图',
      ],
    }),
    language: lang,
...
export function _extractEventAndDataFlow(allFiles, lang, projectPrefix = '') {
  ...
  function _extractWithVariants(key, pattern, category, knowledgeType) {
    ...
    const agentNotes = [
      `你在本项目中实现${pattern.label}时，**必须**使用 ${primaryLabel} 写法`,
      `新代码 **必须** 遵循项目首选的 ${primaryLabel} 写法`,
    ];
    if (missing) {
      agentNotes.push(`你在本项目中使用${pattern.label}时，**必须**实现完整链路（${missing.join('、')}不可缺少）`);
    }
    if (scanResult.variants.length > 1) {
      agentNotes.push(`项目中有 ${scanResult.variants.length} 种${pattern.label}写法 — 新代码统一使用首选写法`);
    }
