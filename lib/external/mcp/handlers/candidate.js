...
/**
 * 统一创建候选的内部方法 — 所有 submit handler 必须经过此函数，
 * 确保 code/reasoning/metadata/context 映射一致。
 *
 * @param {object} candidateService
 * @param {object} item  - 候选数据（扁平字段或 RecipeParser 解析结果）
...
async function _createCandidateItem(candidateService, item, source, extraMeta = {}) {
  const metadata = { ...buildCandidateMetadata(item), ...extraMeta };
  const reasoning = buildReasoning(item);

  // 如果 reasoning 为空对象（缺少 whyStandard），生成默认值
  if (!reasoning.whyStandard) {
    reasoning.whyStandard = item.rationale || item.summary || item.description || 'Submitted via MCP';
...
/**
 * Recipe-Ready 字段完整性检查
 * 返回 { ready, missing[], suggestions[] } 告知 Agent 还需要补哪些字段
 */
function _checkRecipeReadiness(item) {
  const VALID_CATEGORIES = ['View', 'Service', 'Tool', 'Model', 'Network', 'Storage', 'UI', 'Utility'];
  const missing = [];
  const suggestions = [];
  ...
  if (!item.category) {
    ...
    suggestions.push('category 必须为: ' + VALID_CATEGORIES.join('/'));
  } else if (!VALID_CATEGORIES.includes(item.category)) {
    suggestions.push(`category "${item.category}" 非标准值，应为: ${VALID_CATEGORIES.join('/')}`);
  }
...
export async function submitSingle(ctx, args) {
  ...
  // Recipe-Ready 诊断
  const readiness = _checkRecipeReadiness(args);
  const data = { ...result };
  if (!readiness.ready) {
...
export async function submitBatch(ctx, args) {
  ...
  // Recipe-Ready 统计
  const notReady = items.filter(it => !_checkRecipeReadiness(it).ready);
  if (notReady.length > 0) {
    // 汇总所有缺失字段（去重）
    const allMissing = [...new Set(notReady.flatMap(it => _checkRecipeReadiness(it).missing))];
    data.recipeReadyHints = {
