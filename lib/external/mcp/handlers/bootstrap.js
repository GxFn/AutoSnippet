...
import { fillDimensionsAsync } from './bootstrap/pipeline/orchestrator.js';
...
export async function bootstrapKnowledge(ctx, args) {
  ...
  const baseDimensions = [
    // ① 代码规范（Skill）— 新增 api-naming、comment-style 子主题
    { id: 'code-standard', label: '代码规范', guide: '命名约定（类名前缀/方法签名风格/API 命名）、注释风格（语言/格式/MARK 分段）、文件组织规范', knowledgeTypes: ['code-standard', 'code-style'],
      skillWorthy: true, skillMeta: { name: 'project-code-standard', description: 'Project coding standards and naming conventions (auto-generated by bootstrap)' } },
    ...
    // ③ 架构模式（Skill）— 新增 boundary-rules 子主题
    { id: 'architecture', label: '架构模式', guide: '分层架构、模块职责与边界、依赖图、导入约束规则', knowledgeTypes: ['architecture', 'module-dependency', 'boundary-constraint'],
      skillWorthy: true, skillMeta: { name: 'project-architecture', description: 'Project architecture layers, module boundaries and dependency graph (auto-generated by bootstrap)' } },
    ...
    { id: 'project-profile', label: '项目特征', guide: '技术栈、目录结构、三方依赖枚举与用途、Extension/Category 分类聚合、自定义基类层级与全局定义（宏/typealias/PCH）、系统事件 hook 与生命周期入口、基础设施服务注册表、Runtime 与语言互操作', knowledgeTypes: ['architecture'],
      skillWorthy: true, skillMeta: { name: 'project-profile', description: 'Project tech stack, module structure, third-party dependencies, base extensions/classes, event hooks, infrastructure services, and runtime/interop features (auto-generated by bootstrap)' } },
  ...
  const responseData = {
    ...
    // 7 维度分析框架（4 维度 → Skill only, 3 维度 → Candidate only）
    ...
    analysisFramework: {
      ...
      expectedOutput: '50-100 条候选（微观代码维度：code-pattern/best-practice/event-and-data-flow）+ 4 个 Project Skills（宏观叙事维度：architecture/code-standard/project-profile/agent-guidelines）',
    },
  ...
  setImmediate(() => {
    fillDimensionsAsync(fillContext).catch(e => {
      ctx.logger.error(`[Bootstrap] Async fill failed: ${e.message}`);
    });
  });
