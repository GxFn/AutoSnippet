 ...
 * @param {boolean} [args.autoEnrich=false] Phase 6: 自动 AI 补全候选语义字段（需要 aiProvider）
 * @param {boolean} [args.autoRefine=false] Phase 7: 自动 AI 润色候选内容（需要 aiProvider）
 * @param {string}  [args.refinePrompt] 自定义润色提示词（仅 autoRefine=true 时生效）
 */
export async function bootstrapKnowledge(ctx, args) {
  ...
  const shouldEnrich = args.autoEnrich ?? false;
  const shouldRefine = args.autoRefine ?? false;
  ...
  // ═══════════════════════════════════════════════════════════
  // Phase 6: [可选] AI 结构补齐（autoEnrich）
  // 统一链路：ChatAgent 和 MCP 外部 Agent 均通过此参数触发
  // ═══════════════════════════════════════════════════════════
  let enrichResult = null;
  const candidateIds = candidateResults.ids || candidateResults.candidateIds || [];
  if (shouldEnrich && candidateIds.length > 0) {
    try {
      const candidateService = ctx.container.get('candidateService');
      const aiProvider = ctx.container.get('aiProvider');
      if (aiProvider && typeof aiProvider.enrichCandidates === 'function') {
        ctx.logger.info(`[Bootstrap] Phase 6: Enriching ${candidateIds.length} candidates`);
        // 分批处理（每批最多 20 条）
        const batches = [];
        for (let i = 0; i < candidateIds.length; i += 20) {
          batches.push(candidateIds.slice(i, i + 20));
        }
        let totalEnriched = 0;
        for (const batch of batches) {
          try {
            const r = await candidateService.enrichCandidates(batch, aiProvider, { userId: 'bootstrap' });
            totalEnriched += r.enriched || 0;
          } catch (batchErr) {
            ctx.logger.warn(`[Bootstrap] Phase 6 batch failed: ${batchErr.message}`);
          }
        }
        enrichResult = { enriched: totalEnriched, total: candidateIds.length };
        ctx.logger.info(`[Bootstrap] Phase 6 done: ${totalEnriched}/${candidateIds.length} enriched`);
      } else {
        ctx.logger.info('[Bootstrap] Phase 6 skipped: aiProvider not available or missing enrichCandidates()');
      }
    } catch (enrichErr) {
      ctx.logger.warn(`[Bootstrap] Phase 6 failed (non-critical): ${enrichErr.message}`);
      enrichResult = { error: enrichErr.message };
    }
  }
  report.phases.enrich = enrichResult;

  // ═══════════════════════════════════════════════════════════
  // Phase 7: [可选] AI 内容润色（autoRefine）
  // 统一链路：ChatAgent 和 MCP 外部 Agent 均通过此参数触发
  // ═══════════════════════════════════════════════════════════
  let refineResult = null;
  if (shouldRefine && candidateIds.length > 0) {
    try {
      const candidateService = ctx.container.get('candidateService');
      const aiProvider = ctx.container.get('aiProvider');
      if (aiProvider && typeof aiProvider.chat === 'function') {
        // 构建润色提示：优先用调用方提供的 refinePrompt，其次用 Skill 内容
        let userPrompt = args.refinePrompt;
        if (!userPrompt && skillContext?.languageSkill) {
          userPrompt = `请参考以下业界最佳实践标准润色候选，确保 summary 精准、tags 丰富、confidence 合理:\n${skillContext.languageSkill.substring(0, 3000)}`;
        }
        ctx.logger.info(`[Bootstrap] Phase 7: Refining ${candidateIds.length} candidates`);
        const r = await candidateService.refineBootstrapCandidates(
          aiProvider,
          { candidateIds, userPrompt },
          { userId: 'bootstrap' },
        );
        refineResult = { refined: r.refined, total: r.total, errors: r.errors?.length || 0 };
        ctx.logger.info(`[Bootstrap] Phase 7 done: ${r.refined}/${r.total} refined`);
      } else {
        ctx.logger.info('[Bootstrap] Phase 7 skipped: aiProvider not available or missing chat()');
      }
    } catch (refineErr) {
      ctx.logger.warn(`[Bootstrap] Phase 7 failed (non-critical): ${refineErr.message}`);
      refineResult = { error: refineErr.message };
    }
  }
  report.phases.refine = refineResult;

  // ═══════════════════════════════════════════════════════════
  // 组装最终消息
  // ═══════════════════════════════════════════════════════════
  const msgParts = [
    `Bootstrap 完成: ${allFiles.length} files, ${allTargets.length} targets, ${depEdgesWritten} graph edges, ${candidateResults.created} 条候选已创建`,
  ];
  if (skillsEnhanced) msgParts.push('（Skill 增强）');
  if (enrichResult?.enriched) msgParts.push(`，${enrichResult.enriched} 条已 AI 补齐`);
  if (refineResult?.refined) msgParts.push(`，${refineResult.refined} 条已 AI 润色`);
  msgParts.push('。');
  if (skillContext?.loaded?.length) msgParts.push(`已加载 Skills: ${skillContext.loaded.join(', ')}。`);

  responseData.enrichResult = enrichResult;
  responseData.refineResult = refineResult;
  responseData.message = msgParts.join('');
