/**
 * MCP Handlers — Bootstrap 冷启动知识库初始化 (v3)
 *
 * 设计原则：MCP 调用方是外部 AI Agent，不使用项目内置 AI。
 * 本工具做结构数据收集 + 单一职责多条 Candidate 自动创建：
...
export async function bootstrapKnowledge(ctx, args) {
  ...
  responseData.message = `Bootstrap 完成: ${allFiles.length} files, ${allTargets.length} targets, ${depEdgesWritten} graph edges, ${candidateResults.created} 条单一职责候选已创建（v3 拆分）。外部 Agent 可按 filesByTarget 继续补充更细粒度候选。`;
 ...
 * @param {object} args { candidateIds?: string[], dryRun?: boolean }
 */
export async function bootstrapRefine(ctx, args) {
  ...
  const result = await candidateService.refineBootstrapCandidates(
    aiProvider,
    { candidateIds: args.candidateIds, dryRun: args.dryRun },
    { userId: 'cursor_agent' },
  );
