...
export const TOOLS = [
  ...
  // 31. 冷启动知识库初始化（自动创建 9 维度 Candidate）
  {
    ...
    description:
      '项目冷启动：一键初始化知识库（纯启发式，不使用 AI）。覆盖 9 大知识维度（项目规范/代码模式/架构/最佳实践/调用链/数据流/Bug修复/项目特征/Agent注意事项）。\n' +
      ...
      '\n' +
      '⚠️ 产出为启发式初稿，质量不足以直接发布。调用方（你）必须执行以下后续步骤提升质量：\n' +
      '  Step 1: 调用 autosnippet_enrich_candidates — 诊断每条候选的字段缺失，根据返回的 missingFields 逐条补全 category/trigger/summary_cn/summary_en/headers 等必填字段\n' +
      '  Step 2: 调用 autosnippet_bootstrap_refine — AI 润色 summary、补充架构 insight、推断 relations、调整 confidence\n' +
      '  Step 3: 逐 Target 深入分析 filesByTarget，按 analysisFramework.dimensions 补充更细粒度候选（使用 autosnippet_submit_candidates）\n' +
      '  Step 4: 对新提交的候选重复 Step 1-2 确保质量\n' +
      '\n' +
      '质量标准：每条候选必须包含 title/code/language/category/trigger/summary_cn/headers/reasoning，summary 应准确描述代码意图而非泛化模板。',
    inputSchema: {
      type: 'object',
      properties: {
