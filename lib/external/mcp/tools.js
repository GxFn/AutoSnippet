...
export const TOOLS = [
  ...
  // 30. 候选语义字段完整性诊断（不使用内置 AI）
  {
    name: 'autosnippet_enrich_candidates',
    description:
      '诊断已有候选的 Recipe-Ready 字段缺失情况。检查两层：\n' +
      '  ① Recipe 必填：category、trigger(@开头)、summary_cn、summary_en、headers(完整 import)\n' +
      '  ② 语义补全：rationale、knowledgeType、complexity、scope、steps、constraints\n' +
      '返回每个候选缺失了哪些字段，由调用方（外部 Agent）自行补全后重新提交。' +
      '适用场景：候选 → Recipe 之前的查漏补缺。不调用项目内置 AI。',
    inputSchema: {
      type: 'object',
      properties: {
        candidateIds: {
  ...
  // 31. 冷启动知识库初始化（自动创建 9 维度 Candidate）
  {
    ...
    description:
      ...
      '外部 Agent 可根据 filesByTarget 继续按文件粒度补充更多候选，或调用 autosnippet_bootstrap_refine 进行 AI 润色。',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  // 32. Bootstrap 候选 AI 润色（Phase 6）
  {
    name: 'autosnippet_bootstrap_refine',
    description:
      'Phase 6 AI 润色：对 Bootstrap 冷启动生成的候选进行二次精炼。\n' +
      '改善模板化描述、补充架构洞察、推断 relations 关联、调整 confidence 评分。\n' +
      '需要 AI Provider 已配置。建议在 autosnippet_bootstrap_knowledge 之后调用。',
    inputSchema: {
      type: 'object',
      properties: {
