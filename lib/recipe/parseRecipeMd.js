...
/**
 * 检测是否为「仅介绍」Recipe：有 frontmatter（title、trigger）和可选 Usage Guide，不要求代码块。
 * 用于纯介绍类候选，审核后不生成 Snippet。
 */
function isIntroOnlyRecipeMd(text) {
	...
	if (!fm || !fm[1]) return false;
	const meta = parseFrontmatter(fm[1]);
	return !!(meta.title && meta.trigger);
}

/**
 * 解析「仅介绍」Recipe MD：无代码块时返回 ExtractedRecipe 形态，code 为空。
 * 审核时此类候选不生成 Snippet。
 */
function parseIntroOnlyRecipeMd(text) {
	if (!isIntroOnlyRecipeMd(text)) return null;
	const trimmed = text.trim();
	const fmMatch = trimmed.match(FRONTMATTER_RE);
	const meta = parseFrontmatter(fmMatch[1]);
	const body = trimmed.slice(fmMatch[0].length);
	...
	const headers = Array.isArray(meta.headers) ? meta.headers : (meta.headers ? [String(meta.headers)] : []);
	const trigger = (meta.trigger || '').trim().startsWith('@') ? meta.trigger : `@${meta.trigger || 'recipe'}`;
	return {
		title: meta.title || 'Untitled',
		summary: meta.summary || '',
		summary_cn: meta.summary || '',
		...
		trigger,
		category: meta.category || 'Utility',
		language: (meta.language && String(meta.language).toLowerCase()) || 'markdown',
		code: '',
		usageGuide: usageGuide || meta.summary || '',
		usageGuide_cn: usageGuide || meta.summary || '',
		usageGuide_en: usageGuide || meta.summary || '',
		headers,
