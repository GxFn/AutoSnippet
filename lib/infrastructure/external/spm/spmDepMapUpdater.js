...
async function buildSpmProjectGraphFromFullParse(projectRoot, packageSwiftFiles) {
  ...
  packages[pkgName] = {
    ...
    targetsInfo: pkgInfo.targetsInfo && Object.keys(pkgInfo.targetsInfo).length
    ? pkgInfo.targetsInfo
    : undefined,
  };
...
async function updateSpmDepMap(projectRoot, options = {}) {
  ...
  if (options.requireParser) {
  const bin = swiftParserClient.getParserBin(projectRoot);
  if (!bin) {
    return { ok: false, changed: false, path: mapPath, reason: 'parserMissing' };
  }
  }
