 ...
 * 
 * 项目结构约定：
 * ProjectRoot/AutoSnippet/AutoSnippet.boxspec.json
 * ProjectRoot/AutoSnippet/recipes/
 */
...
async function findASSpecPathAsync(filePath) {
  const projectRoot = await ProjectStructure.findProjectRootAsync(filePath);
  if (!projectRoot) return null;
  return ProjectStructure.getBoxspecPath(projectRoot);
}
...
function findASSpecPath(filePath, callback, configPath, configDir) {
  // 使用 ProjectStructure 的同步方法
  const projectRoot = ProjectStructure.findProjectRoot(filePath);
  if (projectRoot) {
  const boxspecPath = ProjectStructure.getBoxspecPath(projectRoot);
  if (fs.existsSync(boxspecPath)) {
    callback(boxspecPath);
    return;
  }
  }
  
  // 没有找到，callback 不调用（保持原有行为）
}
...
async function findProjectRoot(filePath) {
  return await ProjectStructure.findProjectRootAsync(filePath);
}

function findProjectRootSync(filePath) {
  return ProjectStructure.findProjectRoot(filePath);
}
