/**
 * PathFinder - 路径/工程结构查找工具
 * - 定位 AutoSnippet.boxspec.json、Package.swift 等关键文件
 * - 解析 SPM Package.swift（AST-lite）：提取 targets、每个 target 的 dependencies/path/sources 等信息
 * 开发环境：项目根下含 AutoSnippet.boxspec.json 的子目录为知识库目录，其父级为项目根
 */

const fs = require('fs');
const path = require('path');
const fileFinder = require('../filesystem/FileFinder');
...
async function findASSpecPathAsync(filePath) {
	...
	// 向上查找任何 .boxspec.json 文件
	return await fileFinder.findUp(filePath, '.boxspec.json');
}
...
async function findProjectRoot(filePath) {
	...
	// 向上查找 AutoSnippet.boxspec.json（开发环境：位于某子目录内，该目录的父级为项目根）
	const specPath = await searchUpwardForFile(startPath, HOLDER_NAME);
	if (!specPath) return null;
	const knowledgeBaseDir = path.dirname(specPath);
	return path.dirname(knowledgeBaseDir); // 项目根 = 知识库目录的父级
}
...
function findProjectRootSync(filePath) {
	...
	try {
		...
		if (stats.isFile()) currentPath = path.dirname(currentPath);
	} catch (err) {}

	const maxLevels = 20;
	let levels = 0;
	while (levels < maxLevels) {
		const boxspecPath = path.join(currentPath, HOLDER_NAME);
		if (fs.existsSync(boxspecPath)) {
			return path.dirname(currentPath); // 项目根 = 含 spec 目录的父级
		}
		const parentPath = path.dirname(currentPath);
		if (parentPath === currentPath) break;
		currentPath = parentPath;
...
/**
 * 根据任意 spec 文件路径解析出项目唯一的 root spec 路径（开发环境：AutoSnippet/AutoSnippet.boxspec.json）
 * @param {string} specFile - 当前 spec 文件路径
 * @returns {Promise<string|null>} 项目 root spec 路径，与 specFile 同目录或为项目知识库内 spec
 */
async function getRootSpecFilePath(specFile) {
	if (!specFile) return null;
	const dir = path.dirname(specFile);
	const projectRoot = path.dirname(dir);
	return Paths.getProjectSpecPath(projectRoot);
}
