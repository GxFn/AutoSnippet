...
try {
	...
	const findPath = require('../../bin/findPath');
...
class AiFactory {
	...
	static async getProvider(projectRoot) {
		...
		try {
			const specPath = path.join(projectRoot, 'AutoSnippetRoot.boxspec.json');
	...
	/**
	 * 仅解析当前使用的 AI 配置（provider / model），不创建实例，供 UI 展示
	 * @param {string} projectRoot
	...
	static getConfigSync(projectRoot) {
		...
		let provider = process.env.ASD_AI_PROVIDER || 'google';
		let model = process.env.ASD_AI_MODEL || this._defaultModel(provider);
		try {
			const specPath = path.join(projectRoot || '', 'AutoSnippetRoot.boxspec.json');
			if (projectRoot && fs.existsSync(specPath)) {
				const spec = JSON.parse(fs.readFileSync(specPath, 'utf8'));
				if (spec.ai) {
					provider = spec.ai.provider || provider;
