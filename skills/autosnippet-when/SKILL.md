...
|-------------------------|------------------------|------------------------|
| "把这段代码加入知识库" / "提交到 web" / 刚写好一段标准用法 / Agent 起草了 Recipe | **Create Recipe**: submit via Dashboard (Use Copied Code → paste → review → save); do not write to Knowledge directly | **autosnippet-create** |
...
| "帮我检查一下这个文件是否符合规范" / "Guard" | **Guard**: add `// as:guard` in file, save; watch reviews against Recipes | **autosnippet-recipes** (same Recipe content) |
